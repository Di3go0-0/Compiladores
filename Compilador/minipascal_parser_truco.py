...
from minipascal_lexer_truco import tokens
import minipascal_lexer_truco
import sys
...
def p_program_heading(p):
    '''program_heading : PROGRAM ID program_heading_opt_1 SEMICOLON'''

def p_program_heading_opt_1(p):
	'''program_heading_opt_1 : LPAREN identifier_list RPAREN
		| empty'''
...
def p_actual_parameter_list_1(p):
	'''actual_parameter_list_1 : COMMA actual_parameter actual_parameter_list_1
	    | empty'''
...
def p_array_type(p):
	'''array_type : ARRAY LBRACKET index_type array_type_1 RBRACKET OF element_type'''
...
def p_case_statement(p):
	'''case_statement : CASE expression OF case_element case_statement_1 case_statement_opt END'''
...
def p_declaration_part(p):
	'''declaration_part : declaration_part_opt_1 declaration_part_opt_2 declaration_part_opt_3 declaration_part_opt_4'''
...
def p_function_declaration(p):
	'''function_declaration : function_heading SEMICOLON function_declaration_1 SEMICOLON'''
...
def p_output_value_opt_2(p):
	'''output_value_opt_2 : COLON fraction_length
	    | empty'''
...
def p_referenced_variable(p):
    '''referenced_variable : pointer_variable CIRCUMFLEX'''
...
def p_variant_part(p):
    '''variant_part : CASE tag_field ID OF variant variant_part_1'''
...
def p_error(p):
	if VERBOSE:
		if p is not None:
			...
			print ("ERROR SINTACTICO EN LA LINEA: " + str(minipascal_lexer_truco.lexer.lineno))
	else:
